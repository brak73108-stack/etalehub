...
import { getAll as getAllQuotes } from '../db/quotes.js';
...
export default async function renderMoney() {
  ...
  currentInvoices.forEach(inv => {
    ...
    if (inv.status === 'paid' && inv.paidDate && new Date(inv.paidDate) >= monthStart) paidThisMonth += inv.total;
...
export function initMoneyView() {
  ...
  const invRows = currentInvoices.map(inv => `
    <tr class="table-row" onclick="window.location.hash='#/customers/${inv.customerId}'" style="cursor:pointer; transition: background 0.2s;">
      <td style="padding-left: 1.5rem;">
        <div class="font-medium" style="font-size: 1.1rem; margin-bottom: 0.25rem;">${inv.invoiceNumber}</div>
      ...
      <td><div class="font-medium" style="font-size: 1.1rem;">£${inv.total.toFixed(2)}</div></td>
      <td><span class="badge badge-${invStatusColors[inv.status]}">${inv.status.toUpperCase()}</span></td>
      <td>
  ...
  `).join('');
  document.getElementById('invoicesTableBody').innerHTML = invRows || `<tr><td colspan="5" class="text-center" style="padding: 3rem;"><div style="font-size: 2.5rem; margin-bottom: 1rem;">🧾</div><h3 class="text-muted">No invoices found</h3></td></tr>`;
  ...
  const quoteRows = currentQuotes.map(q => `
    <tr class="table-row" onclick="window.location.hash='#/customers/${q.customerId}'" style="cursor:pointer; transition: background 0.2s;">
      <td style="padding-left: 1.5rem;"><span class="font-medium" style="font-size: 1.1rem;">${q.quoteNumber}</span></td>
      <td><span class="text-accent font-medium">${customerMap[q.customerId] || 'Unknown'}</span></td>
      <td><div class="font-medium" style="font-size: 1.1rem;">£${q.total.toFixed(2)}</div></td>
      <td><span class="badge badge-${q.status === 'accepted' ? 'success' : (q.status === 'sent' ? 'info' : 'default')}">${q.status.toUpperCase()}</span></td>
      <td>${q.followUpDate ? new Date(q.followUpDate).toLocaleDateString(undefined, {month:'short', day:'numeric'}) : '-'}</td>
    </tr>
  `).join('');
  document.getElementById('quotesTableBody').innerHTML = quoteRows || `<tr><td colspan="5" class="text-center" style="padding: 3rem;"><div style="font-size: 2.5rem; margin-bottom: 1rem;">📝</div><h3 class="text-muted">No quotes found</h3></td></tr>`;
  ...
  const paymentRows = payments.map(p => `
    <tr class="table-row" onclick="window.location.hash='#/customers/${p.customerId}'" style="cursor:pointer; transition: background 0.2s;">
      ...
      <td><span class="text-muted">${p.invoiceNumber}</span></td>
      ...
      <td><div class="font-medium text-success" style="font-size: 1.25rem;">£${p.total.toFixed(2)}</div></td>
    </tr>
  `).join('');
  document.getElementById('paymentsTableBody').innerHTML = paymentRows || `<tr><td colspan="5" class="text-center" style="padding: 3rem;"><div style="font-size: 2.5rem; margin-bottom: 1rem;">💳</div><h3 class="text-muted">No payments found</h3></td></tr>`;
}
